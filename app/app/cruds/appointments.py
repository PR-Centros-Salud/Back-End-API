...
from schemas.appointments import (
    ...
    AppointmentCreate,
    AppointmentUpdate,
    MedicalAppointmentFinished,
    LaboratoryAppointmentFinished,
    MedicalAppointmentCreate,
    LaboratoryAppointmentCreate,
)
...
def create_medical_appointment(
    db: Session,
    appointment: Union[MedicalAppointmentCreate, LaboratoryAppointmentCreate],
):
    try:
...
def update_appointment(
    db: Session, id: int, appointment_status: int, user: PersonGet, type: int
):
    try:
        db_appointment = None
        ...
        if type == 1:
            db_appointment = (
                ...
                .filter(and_(MedicalAppointment.id == id))
                .first()
            )
        else:
            db_appointment = (
                db.query(LaboratoryAppointment)
                .filter(and_(LaboratoryAppointment.id == id))
                .first()
            )
        ...
        if appointment_status == 2 and (
            db_appointment.status == 3 or db_appointment.status == 4
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        if (
            ...
            and user.id != db_appointment.patient_id
            and user.id != db_appointment.medical_personal_id
        ):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
def finish_appointment(
    ...
    id: int,
    user: PersonGet,
    finished: Union[MedicalAppointmentFinished, LaboratoryAppointmentFinished],
):
    try:
        db_appointment = None
    ...
    try:
        ...
        if type == 0:
            db_appointment = (
                ...
                .filter(
                    and_(
                        MedicalAppointment.id == id,
                        or_(
                            MedicalAppointment.patient_id == user,
                            MedicalAppointment.medical_personal_id == user,
                        ),
                    )
                )
                .first()
            ...
            db_appointment = (
                ...
                .filter(
                    and_(
                        LaboratoryAppointment.id == id,
                        or_(
                            LaboratoryAppointment.patient_id == user,
                            LaboratoryAppointment.medical_personal_id == user,
                        ),
                    )
                )
                .first()
            ...
            db_appointment.laboratory_service = (
                db.query(LaboratoryService)
                .filter(
        ...
        db_appointment.medical_personal = (
        ...
        db_appointment.schedule_day_appointment = (
...
def get_medical_personal_appointments(
    db: Session, medical_personal_id: int, appointment_status: int, type: int
):
    try:
        ...
        if type == 1:
            if appointment_status == 5:
                ...
                db_appointments = (
                    ...
                    .filter(
                        and_(
                            MedicalAppointment.medical_personal_id
                            == medical_personal_id,
            ...
            for appointment in db_appointments:
                appointment.patient = (
                    db.query(Client).filter(Client.id == appointment.patient_id).first()
                )
                appointment.room = (
                    db.query(Room).filter(Room.id == appointment.room_id).first()
                )
                appointment.schedule_day_appointment = (
                    db.query(ScheduleDayAppointment)
                    .filter(
            ...
            if appointment_status == 5:
                ...
                db_appointments = (
                    ...
                    .filter(
                        and_(
                            LaboratoryAppointment.medical_personal_id
                            == medical_personal_id,
            ...
            for appointment in db_appointments:
                ...
                appointment.laboratory_service = (
                    db.query(LaboratoryService)
                    .filter(LaboratoryService.id == appointment.laboratory_service_id)
                    .first()
                )
        ...
        if type == 1:
            ...
            for ap in db_appointments:
                ...
                ap.schedule_day_appointment = (
                    db.query(ScheduleDayAppointment)
                    .filter(ScheduleDayAppointment.id == ap.schedule_day_appointment_id)
                    .first()
                )
                ...
                ap.institution = (
                    ...
                    .filter(Institution.id == ap.institution_id)
                    .first()
                )
            return db_appointments
        else:
            ...
            for ap in db_appointments:
                ap.room = db.query(Room).filter(Room.id == ap.room_id).first()
                ap.laboratory_service = (
                    db.query(LaboratoryService)
                    .filter(LaboratoryService.id == ap.laboratory_service_id)
                    .first()
                )
                ap.medical_personal = (
                    db.query(MedicalPersonal)
...
def get_available_times(db: Session, doctor_id: int, date_time: date):
    try:
        db_contract = (
            db.query(Contract).filter(Contract.medical_personal_id == doctor_id).first()
        )
        db_schedule = (
            db.query(Schedule).filter(Schedule.id == db_contract.schedule_id).first()
        )
        ...
        if db_schedule_day is not None:
            db_schedule_day_appointment = (
                db.query(ScheduleDayAppointment)
                .filter(ScheduleDayAppointment.schedule_day_id == db_schedule_day.id)
                .all()
            )
